...
interface IModalCompany {
    ...
    onHide: () => void,
    currentData: any,
    refetchLstCompany: any
}

const ModalCompany = ({ action, onHide, currentData, refetchLstCompany }: IModalCompany) => {
    ...
    const { isLoading: loadingCreateCompany, mutate: mutateCreateCompany, } = useMutation(
        ...
        onSuccess: (res) => {
            if (res.status === 200) {
                showAlert.success('Tạo doanh nghiệp thành công');
                refetchLstCompany();
                onHide();
            } else {
    ...
    const { isLoading: loadingEditCompany, mutate: mutateEditCompany, } = useMutation(
        ...
        onSuccess: (res) => {
            if (res.status === 200) {
                showAlert.success('Cập nhật doanh nghiệp thành công');                
                refetchLstCompany();
                onHide();
            } else {
                showAlert.error(res.message || 'Đã có lỗi xảy ra, vui lòng thử lại')
    ...
    return (
        <Modal
            title={action === 'create' ? 'Tạo mới doanh nghiệp' : 'Cập nhật thông tin doanh nghiệp'}
            open={!!action}
            keyboard={true}
